...
def plot_bars_2gr_diff(axs, ctrl, nested_cases, relative, sens_cases, var, versions, versions_dic):
    ...
    axs[0].set_ylabel(imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_var_name_xr(d_xr, var) +
                      ' [%s]' % imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_unit_xr(d_xr, var))#, fontsize=FONT_SIZE)
    if relative:
        axs[1].set_ylabel(imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_var_name_xr(d_xr, var) + ' [%]')#,
                          #fontsize=FONT_SIZE)
    else:
        axs[1].set_ylabel(imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_var_name_xr(d_xr, var) +
                          ' [%s]' % imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_unit_xr(d_xr, var))#, fontsize=FONT_SIZE)
