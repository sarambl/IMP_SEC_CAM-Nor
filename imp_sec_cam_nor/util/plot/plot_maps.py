...
import imp_sec_cam_nor.util.naming_conventions.var_info
# import analysis_tools.fix_xa_dataset as fix_xa_dataset
# from imp_sec_cam_nor.util.plot import  plot_settings
from imp_sec_cam_nor.data_info import get_nice_name_case, get_area_specs
from imp_sec_cam_nor.util.slice_average.area_mod import lon180_2lon360
from imp_sec_cam_nor.util.slice_average.avg_pkg import average_model_var  # , get_lat_wgts_matrix, masked_average
# import analysis_tools.area_pkg_sara as area_pkg_sara
import numpy as np
import matplotlib.ticker as mticker
from imp_sec_cam_nor.util.naming_conventions.var_info import get_fancy_var_name
...
def subplots_map(*vars, **kwargs):
    ...
    if 'subplot_kw' not in kwargs:
        ...
        kwargs['subplots_kw']['projection'] = ccrs.Robinson()
...
def plot_map(var, case, cases_dic, figsize=None,
    ...
    label = get_fancy_var_name(var) + ' [%s]' % imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_unit_xr(plt_var,
                                                                                                                var)
    ...
    unit = imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_unit_xr(plt_var, var)
...
def plt_map(plt_var, ax=None, figsize=None, **kwargs_abs):
    ...
    #plt_var.plot()
    #plt.show()
    plt_var.plot(ax=ax, transform=ccrs.PlateCarree(), **kwargs_abs)  # vmin=vmin, vmax=vmax)
...
def plot_map_diff(var, case_ctrl, case_oth, cases_dic, figsize=None, relative=False,tit_ext='',
    ...
    if relative:
        ...
        func = fdifference
        unit_diff = ' [%s]' % imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_unit_xr(ctrl_da, var)
    ...
    if contourf:
        print(kwargs_diff)
        im = plt_var.plot.contourf(ax=ax, transform=ccrs.PlateCarree(), **kwargs_diff)
    else:
...
def get_vmin_vmax(plt_not_ctrl, quant=0.01):
    ...
    if vmin < 0 and vmax > 0:
        ...
        vmin = -minmax
        vmax = minmax
    if type(vmax) is xr.DataArray:
        vmax = float(vmax.values)
    if type(vmin) is xr.DataArray:
            vmin = float(vmin.values)
    if type(vmin) is not None:
        if type(vmax) is not None:
            if vmax<vmin:
                vmax=vmin
                #vmin=None
                #vmin=None
...
def get_avg_diff(case_oth, case_ctrl, case_dic, relative, var):
    ...
    if relative:
        ...
        unit = imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_unit_xr(case_dic[case_ctrl][var], var)
