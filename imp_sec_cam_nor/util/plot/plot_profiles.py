...
from imp_sec_cam_nor.data_info import get_area_defs_pd
...
from imp_sec_cam_nor.util import practical_functions
from imp_sec_cam_nor.util.plot.colors import get_area_col


# %%

def plot_profile_multivar(ds, case, var, ax, xscale='log', yscale='log', label='',
                          ylim=[1e3, 100], pressure_coords=True, kwargs={}, title=''):
...
def plot_profile(da, ax, xscale='log', yscale='log',
    ...
    if pressure_coords:
        ax.set_ylabel('Pressure [hPa]')
    var = da.name
    xlabel = get_fancy_var_name(var) + ' [%s]' % imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_unit_xr(da,
                                                                                                                 var)
    ax.set_xlabel(xlabel)
    ax.grid(True, which='both')


def plot_var_multicase(ax, var, nested_cases, cases, pltkwargs={}, area='', area_in_label=False):
    ...
    xlab = '%s [%s]' % (
    imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_var_name_xr(nested_cases[case][var], var),
    imp_sec_cam_nor.util.naming_conventions.var_info.get_fancy_unit_xr(nested_cases[case][var], var))
    ax.set_xlabel(xlab)


def plot_var_multicase_sets_cases(caselistlist, list_nested_cases, plot_vars_list, nr_rows=2,
                                  figsize=[12, 8], xscale='log', title='', ylim=[1e3, 100], axs=None,
...
def plot_profiles_multivar_cases(axs, nested_cases, cases, plot_vars, xlim=None, xscale='log', kwargs={},
                                 title=''):
...
def plot_profiles_multivar_sets_cases(caselistlist, list_nested_cases, plot_vars_list, figsize=[12, 8],
                                      xscale='log', title=''):
    """
    Ex:
