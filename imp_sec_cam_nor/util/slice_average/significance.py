...
from imp_sec_cam_nor.util.slice_average.avg_pkg import yearly_mean_dic
# %%
# %%
...
def calc_sign_over_fields(da1, da2, avg_dim='time', ci=.95):
    """
    Source: Modern Mathematical Statistics with Applications by Devore & Berk
    Assumptions:
    - data consists of independently selected pairs (X1,Y1)
...
def hatch_area(TF,ax, hatches=None, hatch_lw = .5, reverse=False, transform=None):
    ...
    ax.contourf( x_coord,y_coord, TF.where(TF), hatches=hatches, transform=transform, alpha=0)


def load_and_plot_sign(to_case,from_cases, saxs, var, startyear, endyear, pressure_adjust=True,
                                                                      avg_over_lev=True,
                                                                      ci=.95,
                                                                      groupby=None,
                                                                      dims=('lev',),
