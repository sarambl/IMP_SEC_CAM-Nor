import imp_sec_cam_nor.util.slice_average.avg_pkg as avg_pkg
import imp_sec_cam_nor.util.imports.get_pressure_coord_fields
from imp_sec_cam_nor.util.slice_average.avg_pkg import maps
import imp_sec_cam_nor.util.slice_average.avg_pkg.levlat
import imp_sec_cam_nor.util.slice_average.avg_pkg.one_value
import imp_sec_cam_nor.util.slice_average.avg_pkg.profiles
from imp_sec_cam_nor.util.imports.get_fld_fixed import get_field_fixed
from imp_sec_cam_nor.util.imports.import_fields_xr_v2 import import_constants
import numpy as np
import xarray as xr

from imp_sec_cam_nor.constants import get_input_datapath
# %%
    ...
    for model in models:
        ...
        for case in cases:
            ...
            for var in varList:
                ...
                #    print('save')
                #    print(save_ds)
                #ds_map['lat'] = np.around(ds_map['lat'].values, decimals=2)

                #ds_map['lon'] = np.around(ds_map['lon'].values, decimals=2)
                if first:
                    save_ds = ds_map.copy()
                    first = False
                else:
...
def get_area_avg_nested(models, cases, varList, area, from_time, to_time,
    ...
    for model in models:
        ...
        for case in cases:
            ...
            for var in varList:
                # Check if data already calculated:
                ds_avg, var_loaded = imp_sec_cam_nor.util.slice_average.avg_pkg.one_value.load_area_file_var(model,
                                                                                                          case,
                                                                                                          var,
                                                                                                          area,
                                                                                                          from_time,
                ...
                if not var_loaded:
                    ...
                    ds_avg = imp_sec_cam_nor.util.slice_average.avg_pkg.one_value.get_average_area(dummy,
                                                                                                [var], case,
                                                                                                area, from_time,
                                                                                                to_time, model, pmin,
