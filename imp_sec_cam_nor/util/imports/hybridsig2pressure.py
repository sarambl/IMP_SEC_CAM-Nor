...
from imp_sec_cam_nor.util.filenames import get_filename_pressure_coordinate_field
from imp_sec_cam_nor.util.practical_functions import extract_path_from_filepath, make_folders
...
def hybsig2pres_vars(ds, _vars=None, save_field=True):
    ...
    if _vars is None:
        _vars = list(set(ds.data_vars) - set(imp_sec_cam_nor.data_info.variable_info.not_pressure_coords))
    ...
    for var in _vars:
        ...
        if 'lev' not in da.dims or var in imp_sec_cam_nor.data_info.variable_info.not_pressure_coords:
            log.ger.info('Saved:' + var)
...
def save_pressure_coordinate_field(ds, var,
    ...
    for key in check_list:
        if ds[var].attrs[key] is True or ds[var].attrs[key] == 'True':
            check = True
        elif var in imp_sec_cam_nor.data_info.variable_info.not_pressure_coords:
            check = True
